import { AIProvider, Column, Connection, Database, Schema, Settings, Table } from "@/types";
...
/**
 * Get the configured AI provider and API key from settings.
 * Prefers Anthropic if both are configured, falls back to OpenAI.
 */
function getAIConfig(settings: Settings): { apiKey: string; provider: AIProvider } {
  if (settings.aiFeatures.providers.anthropic.enabled && settings.aiFeatures.providers.anthropic.apiKey) {
    return {
      apiKey: settings.aiFeatures.providers.anthropic.apiKey,
      provider: "Anthropic",
    };
  }
  if (settings.aiFeatures.providers.openai.enabled && settings.aiFeatures.providers.openai.apiKey) {
    return {
      apiKey: settings.aiFeatures.providers.openai.apiKey,
      provider: "Open AI",
    };
  }
  throw new Error("No AI provider configured with a valid API key");
}

/**
 * Progress tracker for indexing jobs
 */
class IndexProgressTracker {
  private jobId: number;
  private itemsProcessed: number = 0;
  private itemsToProcess: number = 0;
  ...
  async setTotal(total: number) {
    ...
    await updateIndexJob(this.jobId, { itemsToProcess: total });
  }

  async increment() {
    this.itemsProcessed++;
    await updateIndexJob(this.jobId, { itemsProcessed: this.itemsProcessed });
  }

  getProgress() {
    return { processed: this.itemsProcessed, total: this.itemsToProcess };
  }
}
...
const BATCH_CONFIG = {
  ...
  batchSize: 5,
  // Delay between batches in milliseconds (helps avoid rate limits)
...
async function processBatch<T, R>(
  ...
  delayMs: number = BATCH_CONFIG.delayBetweenBatches
...
export async function indexColumn(
  ...
  progressTracker?: IndexProgressTracker
): Promise<Column> {
  ...
  if (
    ...
    const prompt = COLUMN_DESCRIPTION_PROMPT.replace(
      "{connectionDescription}", connectionDescription || "Unknown"
    )
    ...
    if (progressTracker) await progressTracker.increment();
    return updated;
  }
...
export async function indexTable(
  ...
  // Process columns in batches to avoid rate limits
  const results = await processBatch(columns, (c) =>
    indexColumn(
      table,
      c,
      settings,
  ...
  if (autoGenerateDescriptions && (!table.description || !existingTable)) {
    const prompt = TABLE_DESCRIPTION_PROMPT.replace(
      ...
      .replace("{schemaName}", schemaDef.name)
      .replace("{tableName}", tableDef.table)
...
export async function indexSchema(
  ...
  const results = await processBatch(tables, (t) =>
    indexTable(schema, schemaEntity, t, settings, connectionDescription, autoGenerateDescriptions, progressTracker)
  );
...
export async function indexDatabase(
  ...
  // Process schemas in batches to avoid rate limits
  const results = await processBatch(schemas, (s) =>
    indexSchema(databaseEntity, s, settings, connection.description || "Unknown", autoGenerateDescriptions, progressTracker)
  );
  ...
  if (
    ...
    const prompt = DATABASE_DESCRIPTION_PROMPT.replace(
      "{connectionDescription}", connection.description || "Unknown"
    )
...
export async function indexConnection(
  ...
  const progressTracker = new IndexProgressTracker(job.id);
  ...
  try {
    ...
    // Count total columns to process for progress tracking
    let totalColumns = 0;
    for (const dbName of databases) {
      try {
        const { schemas } = await getFullSchema(connection, dbName);
        ...
        console.warn(`Could not count columns for database ${dbName}:`, e);
      }
    }
    await progressTracker.setTotal(totalColumns);
    console.log(`Total columns to index: ${totalColumns}`);

    const settings = await getSettings();
    
    // Process databases in batches to avoid rate limits
    const indexedDatabases = await processBatch(databases, (i) =>
      indexDatabase(connection, i, settings, autoGenerateDescriptions, progressTracker)
    );
