...
export async function indexSchema(
  ...
  const promises = tables.map((t) =>
    indexTable(schema, schemaEntity, t, settings, connectionDescription, autoGenerateDescriptions)
  );
...
export async function indexDatabase(
  ...
  const promises = schemas.map((s) =>
    indexSchema(databaseEntity, s, settings, connection.description || "Unknown", autoGenerateDescriptions)
  );
